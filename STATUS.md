...
- [ ] **Step 4 – Config-backed Authentication & Access Control**  
  ...
  - **Pending Step 4.3 sub-steps (per 2026-03-29 security review in `docs/REVIEW.md`)**  
    ...
    - [ ] **Step 4.3.2 – Shared password hashing upgrade**: replace raw SHA-256 with PBKDF2 (Workers-native), fix `constantTimeEquals` to avoid length timing leaks, document the hashing policy, and add targeted unit tests for variable-length comparisons + regression vectors. Tests: `pnpm test` (password-utils suite) + targeted benchmark note.
