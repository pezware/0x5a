...
- [ ] **Step 4 – Config-backed Authentication & Access Control**  
  ...
  - **Pending Step 4.3 sub-steps (per 2026-03-29 security review in `docs/REVIEW.md`)**  
    - [ ] **Step 4.3.1 – JWT middleware hardening + contract tests**: add tenant ID validation against `config.tenants`, enforce `Allow: POST` on `/sessions/shared-password`, guard `ttlSeconds` in `createSessionToken`, and extend handler/token tests to decode JWT payloads + negative paths (invalid roles, method guard, secret missing). Tests: `pnpm test` (auth handler + session token suites) plus `npm run harness:smoke` before merge.
