...
- [ ] **Step 4 – Config-backed Authentication & Access Control**  
  ...
  - 2026-03-29: Step 4.2 — implemented `/sessions/shared-password` with password hashing (`src/auth/password-utils.js`), HS256 session tokens (`src/auth/session-token.js`), config/schema doc updates, and comprehensive handler tests. Tests: `pnpm test` (adds password/token suites + auth handler cases). Reviewer: Godel (sub-agent) — LGTM.
  - **Pending Step 4.3 sub-steps (per 2026-03-29 security review in `docs/REVIEW.md`)**  
    - [x] **Step 4.3.1 – JWT middleware hardening + contract tests**: 2026-03-29 — tenant ID validation now enforces membership in `config.tenants`, `/sessions/shared-password` returns `Allow: POST` on 405s, `createSessionToken` rejects non-positive `ttlSeconds`, and the handler/token suites decode JWT payloads (covering invalid roles, method guards, missing secrets, and unknown tenants). Tests: `pnpm test`. Reviewer: Maxwell (Gemini CLI) — confirmed coverage matches the action plan.
    - [x] **Step 4.3.2 – Shared password hashing upgrade**: 2026-03-29 — replaced SHA-256 with PBKDF2-SHA256 (600k iterations, salt `shared-password:v1`), exported a true constant-time compare, added the CLI helper (`scripts/hash-shared-password.mjs`), and documented the hashing policy in `docs/auth-access.md`. Tests: `pnpm test` (covers password-utils + auth handler). Reviewer: Maxwell (Claude sub-agent) — confirmed the PBKDF2 params, CLI helper, and regression tests align with the action plan.
    - [ ] **Step 4.3.3 – Health endpoint + observability lockdown**: split `/health` into public liveness vs authenticated diagnostics, redact issuer/audience/secrets, and capture operational expectations in `docs/auth-access.md`. Tests: `pnpm test` (auth handler) + manual `wrangler dev` probe script logged in `docs/harness/health-check.md`.
